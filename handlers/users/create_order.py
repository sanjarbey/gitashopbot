...
from data.products import python_book, ds_praktikum,shoploti,  FAST_SHIPPING, REGULAR_SHIPPING, PICKUP_SHIPPING
...
async def show_invoices(message: types.Message):
    ...
    await message.answer_photo(photo="https://dastur.uz/wp-content/uploads/2023/04/0604-768x768.jpg",
...
@dp.message_handler(Command("shoptoli"))
async def show_invoices(message: types.Message):
    caption = "<b>shoptoli</b> kitobi.\n\n"
    caption += "shoptolio’zbek tilidagi mukammal tuzilgan qo’llanmalardan biridir.\n\n"
    caption += "Qo’lingizdagi shoptolining o’ziga xos jihati shundaki, uning har bir bo’limi uchun tayyorlangan qo'shimcha onlayn"
    caption += "materiallar, jumladan, 50 dan ortiq video darslar, amaliy mashg’ulotlar va vazifalarning kodlari e’tiboringizga havola etilgan.\n\n"
    caption += "O’quvchilar bu materiallarni maxsus QR kod yordamida o’z komputerlariga yuklab olib, ulardan unumli foydalanishi mumkin.\n\n"
    caption += "Narxi: <b>150000 so'm</b>"
    await message.answer_photo(photo="https://pbs.twimg.com/media/DumLTKMWsAAstYr.jpg",
                         caption=caption, reply_markup=build_keyboard("shoptoli"))
...
@dp.callback_query_handler(text="product:shoptoli")
async def praktikum_invoice(call: CallbackQuery):
    await bot.send_invoice(chat_id=call.from_user.id,
                           **shoploti.generate_invoice(),
                           payload="payload:shoptoli")
    await call.answer()
